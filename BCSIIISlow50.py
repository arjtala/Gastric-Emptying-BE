...
for j in range(1):
	...
	for k in range(1):
		...
		patientKgeParams = randKgeParam[k+10]
		patientLagParams = randLagParam[k+10]
...
for j in range(1):
	for k in range(1):
		...
		np.savetxt(fnameConc + '_Elim' + repr(j+1) + '_P_' + repr(k+11) + '.csv', MresultList50[(j)*24+k], delimiter=',')
		np.savetxt(fnamecMax + '_Elim' + repr(j+1) + '_P_' + repr(k+11) + '.csv', cMaxList50[(j)*24+k], delimiter=',')
		np.savetxt(fnametMax + '_Elim' + repr(j+1) + '_P_' + repr(k+11) + '.csv', tMaxList50[(j)*24+k], delimiter=',')
		np.savetxt(fnameAUC + '_Elim' + repr(j+1) + '_P_' + repr(k+11) + '.csv', AUCList50[(j)*24+k], delimiter=',')
