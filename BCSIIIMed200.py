...
title = 'BCS III Med $200mL$'
perm0,perm1,perm2,perm3,perm4,perm5,perm6 = np.concatenate((np.ones(3)*0.5,0.00*np.zeros(4)))
...
for j in range(5):
	for k in range(24):
		print repr(MresultList200[(j)*2+k].shape)
...
print("--- Completed! Total execution time: %s seconds ---" % float(time.time() - float(start_time)))
